...
Migrations apply automatically in lex order via `prisma migrate deploy`,
which `vercel-build` runs on every deploy. There's no manual phasing
or operator intervention. The 4 M3 migrations land together:

| # | Migration | What |
|---|---|---|
| 1 | `20260514160000_m3_bot_handle_add` | Add `handle` + `display_name`, backfill, index swap. Preflight `RAISE EXCEPTION` aborts on cross-owner name collision. |
| 2 | `20260514160100_m3_drop_bot_name` | `DROP COLUMN bots.name`. Irreversible. |
| 3 | `20260514160200_m3_audit_actor_kind` | Add `admin_audit_events.actor_kind` enum + index. |
| 4 | `20260514180000_m3_audit_actor_kind_screaming_case` | Rename actor_kind enum values to SCREAMING_CASE. |

MVP-stage policy: destructive rollouts are acceptable, no external
consumers exist yet, so the `/events` shape change (`bot_name` →
`bot_handle`) ships as a hard cut without a verification probe. If a
post-deploy regression lands, the rollback path is Neon PITR — restore
from a snapshot taken before the deploy, redeploy the pre-M3 SHA,
accept the lost write window. The audit log + replay path can
reconstruct canvas state from `pixel_events` for any gap.
...
STATUS '{"handle":"travis","display_name":"x"}'                 # reserved (project owner)
```
...
Note: there's no protected-prefix mechanism. `m25-foo` and similar
launch-bot-style handles are valid choices for any owner — uniqueness
is enforced by the DB index, and the M2.5 launch bots already own
their handles.
